...
export const BoardView = ({ tasks, groupBy = 'status' }: BoardViewProps) => {
    ...
    return (
        <div className="flex gap-6 overflow-x-auto pb-4 h-full min-h-[500px]">
            {columns.map(col => (
                <div key={col.id} className="min-w-[300px] w-[300px] flex flex-col bg-gray-50/50 rounded-xl p-3 border border-gray-100">
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2">
                        {col.tasks.map(task => (
                            <div key={task.id} className="bg-white rounded-lg shadow-sm border border-gray-100 p-2">
                                <TaskItem task={task} />
                            </div>
                        ))}
                        {col.id === 'todo' && (
